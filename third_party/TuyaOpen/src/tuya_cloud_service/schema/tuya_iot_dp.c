...
static void dp_sync_cb(int result, void *user_data)
{
    ...
    /* Keep the cloud acknowledgement observable without logging the device
     * id or DP payload.  A synchronous OPRT_OK only means the report was
     * queued; this callback is the evidence that the cloud accepted it. */
    PR_INFO("DP cloud sync result: %d", result);
