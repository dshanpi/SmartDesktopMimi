...
#define FREE_CHAT_ECHO_GUARD_MS    (1200)
#define FREE_CHAT_BARGE_IN_GUARD_MS (700)
...
static SYS_TIME_T      sg_audio_guard_until_ms = 0;
static SYS_TIME_T      sg_barge_in_after_ms = 0;
...
{
    /* Free Chat is already awake.  Treating another KWS hit as a new wake-up
     * would stop the foreground TTS and make room noise appear to interrupt
     * the assistant. */
    if (sg_is_wakeup || ai_audio_player_is_playing()) {
        PR_DEBUG("ignore KWS while free chat is active");
        return;
    }
...
static void __ai_mode_enter_listen(void)
{
    ...
    /* The product UI has an explicit exit_free_chat command.  Do not silently
     * fall back to wake-word-only mode after 30 seconds while that page is
     * still open; users reasonably expect Free Chat to keep listening. */
    tal_sw_timer_stop(sg_enter_idle_timer);

    sg_is_wakeup = true;
    /* Normal speech should be easy to detect once the assistant is quiet. */
...
static void __ai_mode_enter_think(void)
{
    ...
    /* Free Chat intentionally runs half-duplex.  Keeping VAD armed while the
     * cloud is thinking lets ambient noise open another upload just as TTS
     * starts, which then interrupts the answer. */
    ai_audio_input_wakeup_set(false);
    tuya_ai_input_stop();
...
static void __ai_mode_enter_speak(void)
{
    ...
    tal_sw_timer_stop(sg_enter_idle_timer);
    /*
     * Keep only the local AEC/VAD path armed while TTS is playing.  A HIGH
     * threshold plus a short opening guard rejects the loud leading edge of
     * the speaker reference, while still allowing a nearby user to interrupt
     * a long answer.  Cloud upload remains stopped until that qualified VAD
     * edge is handled below.
     */
    tuya_ai_input_stop();
    ai_audio_input_reset();
    tkl_vad_set_threshold(TKL_AUDIO_VAD_HIGH);
    sg_barge_in_after_ms = tal_system_get_millisecond() +
                           FREE_CHAT_BARGE_IN_GUARD_MS;
    ai_audio_input_wakeup_set(true);
}
...
static void __ai_mode_enter_idle_time_cb(TIMER_ID timer_id, void *arg)
{
    ...
    /* This timer guards a stalled THINK transition.  Recover into continuous
     * listening instead of disabling ordinary speech inside Free Chat. */
    MODE_STATE_CHANGE(sg_mode_set_state, AI_MODE_STATE_LISTEN);
}
    ...
    switch (event->type) {
        ...
        case AI_USER_EVT_TTS_ABORT: {
            MODE_STATE_CHANGE(sg_mode_set_state,
                              (sg_is_wakeup == true) ? AI_MODE_STATE_LISTEN : AI_MODE_STATE_IDLE);
        }
        break;
        ...
        case AI_USER_EVT_PLAY_END:{
            sg_audio_guard_until_ms = tal_system_get_millisecond() +
                                      FREE_CHAT_ECHO_GUARD_MS;
...
static OPERATE_RET __ai_mode_free_vad_change(AI_AUDIO_VAD_STATE_E vad_flag)
{
    ...
    if (AI_AUDIO_VAD_START == vad_flag) {
        ...
        if (sg_mode_set_state == AI_MODE_STATE_SPEAK &&
            ai_audio_player_is_playing()) {
            if (now_ms < sg_barge_in_after_ms) {
                PR_DEBUG("ignore early speaker VAD during barge-in guard");
                return OPRT_OK;
            }

            PR_NOTICE("free chat barge-in accepted");
            ai_audio_player_stop(AI_AUDIO_PLAYER_ALL);
            ai_audio_input_reset();
            tuya_ai_agent_event(AI_EVENT_CHAT_BREAK, 0);
            MODE_STATE_CHANGE(sg_mode_set_state, AI_MODE_STATE_LISTEN);
        ...
        if (sg_mode_set_state != AI_MODE_STATE_LISTEN ||
            now_ms < sg_audio_guard_until_ms) {
            PR_DEBUG("ignore VAD during half-duplex/echo guard");
            tuya_ai_input_stop();
            return OPRT_OK;
        }
        tuya_ai_agent_set_scode(AI_AGENT_SCODE_DEFAULT);
...
OPERATE_RET ai_mode_free_wakeup(void)
{
    ai_audio_player_stop(AI_AUDIO_PLAYER_ALL);
    ai_audio_input_reset();
    tuya_ai_agent_event(AI_EVENT_CHAT_BREAK, 0);
