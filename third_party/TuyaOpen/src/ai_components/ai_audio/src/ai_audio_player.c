...
static uint32_t __s_tts_stream_bytes = 0;
static uint32_t __s_tts_stream_chunks = 0;
...
OPERATE_RET ai_audio_play_tts_stream(AI_AUDIO_PLAYER_TTS_STATE_E state, AI_AUDIO_CODEC_E codec, char *data,  int len)
{
    ...
    case AI_AUDIO_PLAYER_TTS_START:
        PR_DEBUG("audio player -> tts stream start codec=%d", codec);
        __s_tts_stream_bytes = 0;
        __s_tts_stream_chunks = 0;
    ...
    case AI_AUDIO_PLAYER_TTS_DATA:
        ...
        if (data && len > 0 && __s_tts_play_flag) {
            rt = tuya_ai_player_feed(__s_tone_player, (uint8_t *)data, len);
            if (rt != OPRT_OK) {
                PR_ERR("audio player -> tts feed failed rt=%d len=%d after %u bytes/%u chunks",
                       rt, len, __s_tts_stream_bytes, __s_tts_stream_chunks);
            } else {
                __s_tts_stream_bytes += (uint32_t)len;
                __s_tts_stream_chunks++;
            }
        }
        ai_user_event_notify(AI_USER_EVT_TTS_DATA, NULL);
        break;
    case AI_AUDIO_PLAYER_TTS_STOP:
        rt = tuya_ai_player_feed(__s_tone_player, NULL, 0);
        PR_NOTICE("audio player -> tts stream stop bytes=%u chunks=%u eof_rt=%d",
                  __s_tts_stream_bytes, __s_tts_stream_chunks, rt);
    ...
    case AI_AUDIO_PLAYER_TTS_ABORT:
        ...
        rt = tuya_ai_player_feed(__s_tone_player, NULL, 0);
        if (rt != OPRT_OK) {
            PR_ERR("audio player -> tts abort eof failed rt=%d", rt);
        }
