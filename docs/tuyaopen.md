...
TuyaOpen 是上游开源依赖。产品仓将 TuyaOpen 主仓源码和 platform/LINUX vendor 进
`third_party/TuyaOpen/`（删 `.git` 历史）。下载的 Python/uv 工具链 `.tools` 与
A133_B6 受限库属于本地或私有构建输入，不进入公开源码快照。TuyaOpen 与
platform/LINUX 的产品定制已固化进
...
```text
third_party/TuyaOpen/                         # 可再分发源码（主仓+platform/LINUX，无 .git）
...
vendor/tuyaopen-a133-b6-libs/                 # 私有构建输入，不进入公开快照
```
...
当本机已经准备好 `.tools` 和受限依赖时，`prepare_tuyaopen_build_root.sh` 可全程离线并会：
...
```sh
cd /home/ubuntu/AI-DeskTopBox/.tuyaopen-build/apps/tuya.ai/your_chat_bot
./build_a133.sh /home/ubuntu/AI-DeskTopBox/.tuyaopen-build /home/ubuntu/A133-Tina5.0-v0.9
```
...
/home/ubuntu/AI-DeskTopBox/build/tuyaopen/your_chat_bot_QIO_1.0.1.bin
```

## .tools 与 .venv

`.tools/`（uv 0.11.18 + cpython 3.12.13）可以放在本地
`third_party/TuyaOpen/.tools/`，构建时 rsync 进 `.tuyaopen-build/.tools/`，但该目录被
公开发布边界明确忽略。`.tools` 就位时 `export.sh` 会跳过下载；公开源码的新环境需要按
TuyaOpen 上游流程联网安装，或从有权使用的内部依赖包恢复该目录。
...
需联网一次。内部离线构建可用 `./scripts/vendor_tuyaopen.sh --with-uv-cache` 把
`~/.cache/uv` 放进本地 `.tools/uv-cache/`，prepare 会设 `UV_CACHE_DIR` 指向它；该缓存
仍不得加入公开候选。
...
- `/factory/tuya` 由 S95 启动脚本 bind 到 `/overlay/factory/tuya`，因此 A/B OTA 不会覆盖。
