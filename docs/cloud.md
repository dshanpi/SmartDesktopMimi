...
**跨 OTA 持久化**：`/etc/100ask` 和 `/factory/tuya` 由 `aitvbox-data.init`（S95）bind mount 到
...
100ask SDK 是不随公开源码或 SBOM 分发的可选私有 provider。公开版默认编译
`service_cloud_stub.c`，不会读 `/etc/100ask`、连接 MQTT 或申请绑定码。内部构建必须将
`AITVBOX_100ASK_SDK_ROOT` 指向仓库外的绝对路径，并显式设置
`AITVBOX_ENABLE_100ASK_CLOUD=ON`。目录至少包含：

- `include/100ask_iot.h` 与 `src/100ask_iot.c`：SDK 本体；
- `tools/sign_device.sh` 与 `100ask_keys/`：仅限受控开发签名环境，生产由签名服务执行；
- SDK 自身许可证和 notices：内部发布前单独审核，不得借用本项目许可证。
...
1. **编译**：默认公开构建使用 stub 且不链接 Mosquitto；显式私有构建通过后，
   `lv_backend` 链 `libmosquitto.so.1` + `libcurl.so.4`。
