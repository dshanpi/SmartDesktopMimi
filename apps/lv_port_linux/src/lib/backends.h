...
/* Clear both Sunxi framebuffer pages to transparent/opaque black.  This is
 * used by HDMI Display Mode so the higher-z UI layer can contain only the
 * floating control while the video layer remains visible underneath.
 */
#if LV_USE_SUNXIFB
bool sunxifb_set_transparent_overlay(bool transparent);
#else
static inline bool sunxifb_set_transparent_overlay(bool transparent)
{
    (void)transparent;
    return false;
}
