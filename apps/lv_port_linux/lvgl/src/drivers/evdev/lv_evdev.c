...
#define EVDEV_BITS_TO_BYTES(max_code) (((max_code) + 8) / 8)
#define EVDEV_TEST_BIT(bits, code) (((bits)[(code) / 8] >> ((code) % 8)) & 1U)
...
lv_indev_t * lv_evdev_create_fd(lv_indev_type_t indev_type, int fd)
{
    ...
    if(indev_type == LV_INDEV_TYPE_NONE) {
        uint8_t abs_bits[EVDEV_BITS_TO_BYTES(ABS_MAX)] = { 0 };
        if(ioctl(dsc->fd, EVIOCGBIT(EV_ABS, sizeof(abs_bits)), abs_bits) >= 0) {
            /*
             * Type-B touchscreens commonly expose only ABS_MT_POSITION_X/Y.
             * Treat either the legacy or the multi-touch coordinate pair as
             * an absolute pointer.
             */
            bool has_abs_xy = EVDEV_TEST_BIT(abs_bits, ABS_X) &&
                              EVDEV_TEST_BIT(abs_bits, ABS_Y);
            bool has_mt_xy = EVDEV_TEST_BIT(abs_bits, ABS_MT_POSITION_X) &&
                             EVDEV_TEST_BIT(abs_bits, ABS_MT_POSITION_Y);
    ...
    if(indev_type == LV_INDEV_TYPE_POINTER) {
        ...
        else if(ioctl(dsc->fd, EVIOCGABS(ABS_MT_POSITION_X), &absinfo) == 0) {
            dsc->min_x = absinfo.minimum;
            dsc->max_x = absinfo.maximum;
            LV_LOG_INFO("Using ABS_MT_POSITION_X for pointer calibration");
        }
        else {
        ...
        else if(ioctl(dsc->fd, EVIOCGABS(ABS_MT_POSITION_Y), &absinfo) == 0) {
            dsc->min_y = absinfo.minimum;
            dsc->max_y = absinfo.maximum;
            LV_LOG_INFO("Using ABS_MT_POSITION_Y for pointer calibration");
        }
        else {
