...
export default function PCHome() {
        ...
        */
        ...
        if (parsedMessage.type === "answer") {
          ...
          setRemoteSessionDescription(
            ...
            console.log("[Websocket] Remote description set successfully, adding pending candidates", pendingCandidates.current.length);
            // Process any ICE candidates that arrived before the remote description was set
            while (pendingCandidates.current.length > 0) {
              const candidate = pendingCandidates.current.shift();
              if (candidate) {
                activePeerConnection.addIceCandidate(candidate).catch(e => {
                  console.error("[Websocket] Error adding pending ICE candidate", e);
                });
              }
            }
          });
          ...
          if (activePeerConnection.remoteDescription) {
            activePeerConnection.addIceCandidate(candidate).catch(e => {
              console.error("[Websocket] Error adding ICE candidate", e);
            });
          } else {
            console.log("[Websocket] Remote description not set yet, queuing candidate");
  ...
  const setupPeerConnection = useCallback(async () => {
    ...
    pc.ontrack = function (event) {
      ...
      console.log('[RTP] track.id:', event.track.id);
      console.log('[RTP] track.readyState:', event.track.readyState);
      console.log('[RTP] track.muted:', event.track.muted);
      console.log('[RTP] track.enabled:', event.track.enabled);
      ...
      videoTracks.forEach((t, i) => {
        console.log(`[RTP] VideoTrack[${i}]: readyState=${t.readyState}, muted=${t.muted}, enabled=${t.enabled}, label=${t.label}`);
      });

      // Force unmute video track
      if (event.track.kind === 'video') {
        event.track.enabled = true;
        // Chromium-based browsers may otherwise grow a conservative playout
        // buffer that is useful for passive video but noticeable for KVM.
        // The hint is experimental, so keep the assignment capability-based.
  ...
  return (
    <FeatureFlagProvider appVersion={appVersion}>
      ...
      <div className="relative h-full overflow-hidden">
        ...
        <div className={`grid h-full grid-rows-(--grid-layout) select-none ${dark_bg_style_fun(isDark)}`}>
          <FullscreenProvider>
            <BarTop />
